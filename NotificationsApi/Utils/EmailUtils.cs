...
namespace NotificationsApi.Utils;
    ...
    public static string CreateEmailBody(DispatchOrderCommand command)
    {
        var sb = new System.Text.StringBuilder();
			...
			var firstPart = $@"
                <html>
                    ...
                    <body>
						...
						<p>The detection has been carried out by AWS Rekognition Services.
                        </p>
						<hr/>
			...
			foreach (var facialAgePrediction in command.FacialAgePredictions)
			{
				...
				var image = "data:image/png;base64," + Convert.ToBase64String(facialAgePrediction.Face, 0, facialAgePrediction.Face.Length);

				sb.Append("<img src='" + image + "' border='0' style='margin-top: 5px; width: 75px; height: 53px; margin-left: 5px; margin-bottom: 5px;'></td>");
 
				sb.Append($"<td>{facialAgePrediction.AgePrediction}</td>");
